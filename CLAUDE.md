...
Below is for when you're editing the pai-pro repo itself, not running a project session. Skip if the user is asking for filmmaking work. See `skills/maintain-pai-pro/SKILL.md` for adding a media CLI, adding a node type, updating this file, and debugging the viewer/server/pty stack.
